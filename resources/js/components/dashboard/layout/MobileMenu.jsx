...
export default function MobileMenu({ isOpen, onClose, twitterUrl }) {
    ...
    return createPortal(
        <div className="lg:hidden fixed inset-0 z-[9999]" aria-hidden={!isOpen}>
            ...
            />
            <div
                className={`absolute bottom-0 gap-3 left-0 right-0 min-h-[280px] max-h-[85vh] bg-[#FDFDFD] rounded-t-[24px] flex flex-col py-6 px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] overflow-y-auto mobile-menu-panel ${isOpen ? "mobile-menu-open" : "mobile-menu-closed"}`}
                style={{
